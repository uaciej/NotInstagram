...
class ImageUploadView(APIView):
    def post(self, request, format=None):
        ...
        if serializer.is_valid():
            file_ext = os.path.splitext(request.data['file'].name)[1]  # Get the file extension
            if file_ext.lower() not in ['.jpg', '.jpeg', '.png']:
                return Response({'detail': 'Invalid file type. Only .jpg, .jpeg, and .png files are allowed.'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def create_thumbnail(self, image, size):
        ...
        img_format = img.format  # This will be 'JPEG' for .jpg files and 'PNG' for .png files
...
def signed_image_view(request, signed_value):
    ...
    if datetime.now() > datetime.fromtimestamp(int(expiration_time)):
